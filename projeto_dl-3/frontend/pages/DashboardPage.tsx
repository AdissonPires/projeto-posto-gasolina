...
export default function DashboardPage(): JSX.Element {
  ...
  const [data, setData] = useState<BackendDashboardResponse['data'] | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!res.ok) throw new Error(`Falha ao carregar /api/dashboard (${res.status})`);

        const json = (await res.json()) as BackendDashboardResponse;
    ...
    return `
      ...
      <div class="section">
        <div class="section-header">
          <h3>Evolução de Vendas - Últimos 7 dias</h3>
          <div class="actions">
            <button class="btn btn-outline" type="button">7 Dias</button>
            <button class="btn btn-primary" type="button">30 Dias</button>
          </div>
        </div>
        <div class="card">
