...
type SimulatorCalculateResponse = {
  ok: boolean;
  data: {
...
export default function SimulatorPage(): JSX.Element {
  ...
  const [base, setBase] = useState<SimulatorBaseResponse['data']['base'] | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!res.ok) throw new Error(`Falha ao carregar /api/simulator (${res.status})`);

        const json = (await res.json()) as SimulatorBaseResponse;
        if (!json.ok) throw new Error('Resposta inválida da API');
        if (mounted) setBase(json.data.base);
      } catch (e: any) {
        if (!mounted) return;
    ...
    (async () => {
      ...
      try {
        const res = await fetch(`${API_BASE}/api/simulator/calculate`, {
          ...
          body: JSON.stringify({ gasPrice, dieselPrice, ethanolPrice }),
        });

        if (!res.ok) throw new Error(`Falha no calculate (${res.status})`);
        const raw = (await res.json()) as SimulatorCalculateResponse;
        if (!raw.ok) throw new Error('Resposta inválida da API');
        const json = raw.data;
